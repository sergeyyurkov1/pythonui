...
SCREENS_PATH = os.path.join(
    os.path.realpath(os.path.dirname(__file__)), "screens", "user"
)
...
class PersonalTerminal(App):
    ...
    def get_user_screens(self) -> list[str]:
        return sorted(
            [
                i
    ...
    def get_user_subscreens(self) -> list[str]:
        user_subscreens = []
    ...
    def install_user_screens(self):
        for i in self.user_screens:
            screen_module = importlib.import_module(f"screens.user.{i}.main")
